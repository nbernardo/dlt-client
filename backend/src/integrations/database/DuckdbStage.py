    ...
    def get_tables_hierarchy(dbfile, stg_table, stg_db, data_set):
        try:
            ...
            query = f'''
                ...
                SELECT
                    fk.schema AS schema, m.table_name AS "table", fk.schema AS parent_schema, fk."table" AS parent_table,
                    m.fk_col AS fk_column, m.ref_col AS ref_column, fk.depth + 1 AS depth, fk.path_names || ARRAY[fk.schema || '.' || m.table_name]
                ...
                WHERE fk.depth < 5
                    ...
                    AND EXISTS (
                        ...
                        WHERE t.table_name = m.table_name AND t.table_schema = fk.schema AND t.table_catalog = '{stg_db}' AND t.table_schema NOT LIKE '%_staging'
                    )
                ),
                table_columns AS (
