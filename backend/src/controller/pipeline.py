...
def create():
    ...
    data_place = {}
    parse_node(connections, node_params, data_place)
    ...
    pipeline.create_v1(pipeline_name, template)
    return 'From blueprint'


def parse_node(connections, node_params, data_place):
    """
    ...
    for connection in connections:
        for conn in connection:
            ...
            if node_type in ['Start', 'End']:
                ...
                node.run()
                if (len(inner_cnx) > 0):
                    parse_node(inner_cnx, node_params, data_place)
