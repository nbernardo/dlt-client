...
def handle_pipeline_log(printed_log: str, logger: logging.Logger, error = False, warning = False, context = None, display = None):
...
class PipelineLogger:
    ...
    def warning(self, description, extra=None):
        print(f'WARNING: {description} |+| {json.dumps(extra)}')
    ...
    def setup_dlt_logging(self):
        ...
        def _emit(record):
            handle_pipeline_log(record.getMessage(), logger=self, error=record.levelno >= logging.ERROR, warning=record.levelno == logging.WARNING)

        dlt_logger_handler.emit = _emit
        dlt_logger.addHandler(dlt_logger_handler)