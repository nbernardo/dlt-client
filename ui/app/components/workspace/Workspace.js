...
export class Workspace extends ViewComponent {
	...
	buildWorkspaceView() {
		...
		this.controller.on('load', () => {
			...
			this.controller.editor = editor;
			window.draweditor = editor;
			editor.reroute = true;
			...
			this.controller.handleListeners(editor);
		});
	}
	...
	async drop(ev) {
		if (ev.type === "touchend") {
			...
			if (parentdrawflow != null) {
				await this.controller.addNodeToDrawFlow(this.mobileItemSelec, mobile_last_move.touches[0].clientX, mobile_last_move.touches[0].clientY);
			}
			...
			await this.controller.addNodeToDrawFlow(data);
		}
	}
